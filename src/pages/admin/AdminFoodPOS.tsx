...
export default function AdminFoodPOS() {
  ...
  // Extracted components for mobile/desktop reuse
  const MenuGrid = () => (
    <>
      <div className="px-3 sm:px-4 pt-3 sm:pt-4 pb-2 flex items-center gap-3">
      ...
      <Tabs defaultValue="all" className="flex-1 flex flex-col px-3 sm:px-4 overflow-hidden">
              ...
              {getItemsByCategory(cat).map(item => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "relative flex flex-col items-center rounded-xl border-2 p-2 sm:p-3 transition-all hover:shadow-md active:scale-95",
                      qty > 0
                  ...
                  >
                    {qty > 0 && (
                      <Badge className="absolute -top-2 -right-2 h-5 w-5 sm:h-6 sm:w-6 rounded-full p-0 flex items-center justify-center text-[10px] sm:text-xs">
                        {qty}
                      </Badge>
                    )}
                    {item.image_url ? (
                      <img src={item.image_url} alt={item.name} className="h-10 w-10 sm:h-14 sm:w-14 rounded-lg object-cover mb-1.5 sm:mb-2" />
                    ) : (
                      <div className="h-10 w-10 sm:h-14 sm:w-14 rounded-lg bg-muted flex items-center justify-center mb-1.5 sm:mb-2">
                        <UtensilsCrossed className="h-5 w-5 sm:h-6 sm:w-6 text-muted-foreground" />
                      </div>
                    )}
                    <span className="text-xs sm:text-sm font-medium text-center leading-tight line-clamp-2">{item.name}</span>
                    <span className="text-primary font-bold text-xs sm:text-sm mt-0.5 sm:mt-1">৳{item.price}</span>
                  </button>
                );
              })}
            </div>
  ...
  const CartPanel = () => (
    <>
      ...
      {/* Checkout */}
      <div className="border-t border-border px-3 sm:px-4 py-2.5 sm:py-3 space-y-2 sm:space-y-3">
        <Input
      ...
      {/* Recent Orders */}
      <div className="border-t border-border px-3 sm:px-4 py-2 max-h-36 lg:max-h-48 overflow-y-auto">
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] lg:h-[calc(100vh-1rem)] overflow-hidden">
      {/* Mobile: Tab switcher (Menu / Cart) */}
      <div className="lg:hidden flex border-b border-border">
        <button
          onClick={() => setMobileView('menu')}
          className={cn(
            "flex-1 py-2.5 text-sm font-medium text-center transition-colors flex items-center justify-center gap-1.5",
            mobileView === 'menu' ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
          )}
        >
        ...
        <button
          ...
          className={cn(
            "flex-1 py-2.5 text-sm font-medium text-center transition-colors flex items-center justify-center gap-1.5 relative",
            mobileView === 'cart' ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
          )}
        >
          <ShoppingCart className="h-4 w-4" /> Cart
          {cart.length > 0 && (
            <Badge className="h-5 min-w-[20px] rounded-full p-0 flex items-center justify-center text-[10px]">
              {cart.length}
            </Badge>
          )}
        </button>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        {/* Menu Panel */}
        <div className={cn("flex-1 flex flex-col min-w-0 lg:border-r border-border", mobileView === 'menu' ? "flex lg:flex" : "hidden lg:flex")}>
          <MenuGrid />
        </div>

        {/* Cart Panel */}
        <div className={cn("w-full lg:w-[380px] flex flex-col bg-card overflow-hidden", mobileView === 'cart' ? "flex lg:flex" : "hidden lg:flex")}>
          <div className="hidden lg:block px-4 pt-4 pb-2 border-b border-border">
