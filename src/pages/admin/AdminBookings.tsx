...
export default function AdminBookings() {
  ...
  const getTicketTypeBadge = (type: string) => {
    switch (type) {
      case 'child_guardian':
        return <Badge variant="outline">{'Child + Guardian'}</Badge>;
      case 'child_only':
        return <Badge variant="outline">{'Child Only'}</Badge>;
      case 'group':
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedBooking && format(parseISO(selectedBooking.slot_date), 'EEEE, dd MMMM yyyy', { locale: undefined })}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={manualBookingOpen} onOpenChange={setManualBookingOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {'Create a new booking from the counter'}
            </DialogDescription>
          </DialogHeader>
          <ManualBookingForm 
            onSuccess={() => {
