...
export function WhatsAppButton({ variant = "navbar", className }: WhatsAppButtonProps) {
  ...
  const defaultMessage = language === "bn"
    ? "আসসালামু আলাইকুম, Baby World! আমি আপনাদের সেবা সম্পর্কে জানতে চাই।"
    : "Hello, Baby World! I'd like to know more about your services.";
  ...
  if (variant === "navbar") {
    return (
      <a
        ...
        className={`flex items-center gap-2 px-3 py-2 bg-[#25D366] hover:bg-[#128C7E] text-white rounded-lg text-sm font-medium transition-colors ${className}`}
        aria-label="Chat on WhatsApp"
      >
        <WhatsAppIcon className="w-4 h-4" />
        <span className="hidden xl:inline">{t("whatsapp.chat")}</span>
      </a>
    );
  }
  ...
  if (variant === "footer") {
    return (
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className={`inline-flex items-center gap-2 px-4 py-2 bg-[#25D366] hover:bg-[#128C7E] text-white rounded-lg text-sm font-medium transition-colors ${className}`}
        aria-label="Chat on WhatsApp"
      >
        <WhatsAppIcon className="w-4 h-4" />
